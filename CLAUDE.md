...
- [ ] **Téléphone dans `src/_data/site.json` reste fictif** (`02 48 00 00 00`, affiché dans le footer, les mentions légales et le JSON-LD) — à remplacer avant mise en ligne, sans quoi de vrais visiteurs tomberaient sur un numéro qui ne répond pas.
- [x] ~~Email réel~~ — `solybat@gmail.com` communiqué par le client (2026-07-20), mis à jour dans `src/_data/site.json` et `traiter-devis.php` (`$adresseContact`). Attention : l'en-tête technique `From:` de `traiter-devis.php` reste `no-reply@solybat.fr` (placeholder) et ne doit **pas** devenir `solybat@gmail.com` — un serveur mutualisé qui usurpe une adresse gmail.com en "From" se fait généralement bloquer par les filtres anti-spoofing de Gmail. À corriger avec le vrai nom de domaine une fois connu.
...
- [ ] **Nom de domaine réel** dans `src/_data/site.json` (`url`, actuellement un placeholder `https://www.solybat.fr` utilisé pour les balises canonical/Open Graph/JSON-LD/sitemap.xml/robots.txt) — à corriger dès que le domaine définitif est connu.
