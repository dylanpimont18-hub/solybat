...
- [x] ~~Email réel~~ — **`contact@solybat18.fr`** depuis le 2026-08-25 (adresse pro créée chez Hostinger avec le domaine, en remplacement de `solybat@gmail.com`). Présente dans `src/_data/site.json`, `traiter-devis.php` (`$adresseContact`), la plaquette, la carte de visite et la signature mail. Effet de bord heureux : l'expéditeur `no-reply@solybat18.fr` et le destinataire sont désormais sur le même domaine et le même serveur, donc la remise est locale — le risque de filtrage qui pèse sur un envoi vers Gmail disparaît.
...
- [x] ~~Échec d'envoi du formulaire silencieux~~ — corrigé le 2026-08-25. `mail()` était appelé sans vérifier son retour : en cas d'échec, le Warning PHP produisait une sortie **avant** le `header()` de redirection, donc le visiteur tombait sur un message d'erreur brut et sa demande était perdue sans qu'il le sache. Désormais `@mail(...)` avec test du retour, et redirection vers le formulaire avec `?erreur=envoi` qui invite à écrire directement à l'adresse de contact. C'est aussi ce qui permet de tester les chemins de succès en local, où `mail()` échoue toujours faute de SMTP.
...
- [x] ~~Plaquette PDF `/espace-pro`~~ — générée le 2026-07-30 (HTML → PDF via Edge headless `--print-to-pdf`, voir `src/documents/plaquette-solybat.pdf`), 2 pages A4, contenu repris du positionnement/process/corps de métier déjà validés ailleurs sur le site. **Refondue le 2026-08-24** : le client la trouvait pas assez haut de gamme. Désormais **2 pages A4 paysage à 3 sections égales** (99 mm), page 1 composée symétriquement sombre/clair/sombre. Le passage par un dépliant à pli roulé a été abandonné le 2026-08-24, le client ayant écarté les contraintes d'impression — la géométrie reste dans l'historique git. **Le nombre d'opérations ne doit jamais être affiché** (« 100 % » et non « 8/8 »), et toutes les opérations sont listées, plus seulement les trois meilleures ; le passage par une version « 3 pages » était un malentendu, le client voulait 3 volets sur une feuille. Source source versionnée dans `identite-visuelle/plaquette-solybat.html` (la version du 2026-07-30 avait été produite à partir d'un HTML jeté après coup). Couverture typographique anthracite sans photo — choix assumé : les photos disponibles sont des rendus IA, et une pleine page les trahit. Ajout des **8 opérations chiffrées** en page 3, l'argument le plus fort pour une agence, jusque-là absent de la plaquette. Les filets fins remplacent les cartes à coins arrondis, qui faisaient « capture d'écran de site web ». Le PDF n'est pas regénéré par `npm run build` — commande dans `identite-visuelle/README.md`, et `tests/plaquette.test.js` détecte les chiffres périmés. Liée depuis `espace-pro.njk` (le lien de repli et `espace-pro.css` associé ont été supprimés).
