...
- [ ] **Téléphone et email dans `src/_data/site.json` sont fictifs** (`02 48 00 00 00` / `contact@solybat.fr`, affichés dans le footer, les mentions légales et désormais aussi le JSON-LD) — à remplacer par les vraies coordonnées avant mise en ligne, sans quoi de vrais visiteurs tomberaient sur un numéro/email qui ne répond pas.
...
- [ ] Adresse email réelle dans `traiter-devis.php` (`$adresseContact`, actuellement un placeholder `contact@solybat.fr`, à synchroniser avec `site.json` le jour où l'email réel est connu).
