...
## src/index.njk
Accueil : hero-photos (triptyque plein cadre, CTA devis + appel), frise process, aperçu réalisations, pour qui (colonne investisseurs pointe vers `/retour-client/`, "Voir des cas chiffrés"), garanties (sceau motif), CTA final réchauffé ("Discutons de votre projet", boutons appel + devis via `.boutons-groupe`).
...
## src/css/styles.css
Point d'entrée CSS, `@import` de tous les fichiers de composants/pages (inclut désormais `placeholder-photo.css`, `hero-photos.css`, `bandeau-nouveaute.css`, `photos-vrac.css`, `retour-client.css` et `barre-appel.css`).

## src/css/barre-appel.css
Barre d'appel collante mobile (`.barre-appel-mobile`) : masquée par défaut, `position: fixed; bottom: 0` uniquement sous 768px (même palier que `header.css`), fond `--couleur-terracotta-texte`, `padding-bottom` ajouté sur `body` dans la même media query pour que le contenu ne passe pas sous la barre, `env(safe-area-inset-bottom)` pour les mobiles à encoche.
