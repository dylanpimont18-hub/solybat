...
- filtre `dateISO(date)` — date en `AAAA-MM-JJ` pour les `<lastmod>` du sitemap
...
## src/documents/
Fichiers statiques copiés tels quels (`documents/` en sortie). Contient `plaquette-solybat.pdf` (**dépliant 3 volets** : A4 paysage recto-verso, 2 pages PDF, plié en 3 à l'impression ; lié depuis `espace-pro.njk`), **produit dérivé de `identite-visuelle/plaquette-solybat.html`** — ne jamais tenter de modifier le PDF, éditer le HTML puis relancer `msedge --headless=new --no-pdf-header-footer --print-to-pdf` (commande complète dans `identite-visuelle/README.md`). `npm run build` recopie le PDF tel quel mais ne le regénère pas.

## identite-visuelle/
Déclinaisons du logo hors site (pas buildées par 11ty) : `carte-visite-recto.png`/`-verso.png` (1004×650 px/300 dpi, génériques sans nom de personne ni adresse, cf. CLAUDE.md), `signature-email-solybat.html` (bloc HTML table-based à coller dans un client mail, référence `https://www.solybat18.fr/images/signature-sceau.png` — ne s'affichera qu'une fois le site déployé), `signature-email-apercu.png` (rendu de contrôle). Contient aussi `plaquette-solybat.html` (source de `src/documents/plaquette-solybat.pdf`, **2 pages A4 paysage, 3 sections d'égale largeur (99 mm) par page**. Page 1 `[contact] [pourquoi nous] [couverture]`, composée symétriquement sombre/clair/sombre ; page 2 `[corps de métier] [process] [opérations chiffrées]`. Les contraintes d'impression pliée (pli roulé, volet rentrant à 97 mm inversé d'une face à l'autre, gardes autour des pliures) ont été écartées par le client le 2026-08-24 — récupérables dans l'historique git si besoin. **Règle éditoriale : le nombre d'opérations ne doit jamais apparaître** (d'où « 100 % » et non « 8/8 »), et **toutes** les opérations de `retoursClients.json` sont listées. Couverture typographique anthracite — pas de photo, les photos disponibles étant des rendus IA qu'une agence repère. Les volets sont en `overflow: hidden` : **mesurer le remplissage au navigateur avant tout tirage**, un dépassement serait rogné en silence. Le logo se pose tel quel sur l'anthracite : son disque intérieur est crème, seul le pourtour du cercle est détouré, donc **aucune variante inversée n'est nécessaire**. Les chiffres de la page 3 sont écrits en dur et gardés par `tests/plaquette.test.js`) et `publicite-climatisation.html`/`.png` (1080×1350, publicité climatisation réseaux sociaux/impression, générée le 2026-08-01 : reproduction ton/palette Soly'bat d'une publicité concurrente fournie par l'utilisateur, prix "à partir de 2 500 € HT" conservé, mention TVA reprise du bandeau du site — "possible sous conditions" — voir détail dans `identite-visuelle/README.md`). Voir `identite-visuelle/README.md`.
...
## src/_data/site.json
Données globales, toutes réelles depuis 2026-07-20 (communiquées par le client) : `url` (`https://www.solybat18.fr`, utilisé pour canonical/OG/JSON-LD/sitemap.xml), `telephone`/`telephoneLien` (`06 14 59 13 74` / `+33614591374`), `email` (`contact@solybat18.fr`) — utilisées dans le footer, les mentions légales et le JSON-LD. `src/robots.txt` référence aussi le domaine mais n'est pas piloté par ce fichier (passthrough statique, mis à jour séparément).
...
## src/.htaccess
Config Apache pour l'hébergement Hostinger/OVH — copiée tel quelle vers `_site/.htaccess` : **canonicalisation vers `https://www.solybat18.fr`** (le domaine nu et le HTTP répondaient tous deux en 200 sans redirection, donc Google voyait deux sites au contenu identique ; la règle du domaine nu vise l'hôte en dur pour rediriger en **un seul** saut plutôt que d'enchaîner nu→www puis http→https), `ErrorDocument 404 /404.html`, compression (mod_deflate), cache navigateur long (1 an) sur CSS/JS/images via `mod_expires`/`Header`, en-têtes de sécurité de base (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`), type MIME du manifest. Non testable en local (pas de serveur Apache dans l'environnement de dev) — à vérifier après déploiement.

## src/sitemap.njk
Génère `/sitemap.xml` (18 URLs). Deux sources, et c'est indispensable : `collections.all` filtré sur les entrées **non paginées**, puis `realisations.json` énuméré à part. 11ty ne place dans ses collections que la **première** page d'un template paginé — un sitemap basé sur `collections.all` seul ne listait qu'1 des 6 fiches projet, les 5 autres existaient dans `_site/` mais restaient invisibles pour Google. Le filtre sur `pagination` évite de dupliquer cette première fiche. `<lastmod>` via le filtre `dateISO`. 404 et devis-merci sont exclues d'office (`eleventyExcludeFromCollections`, cohérent avec leur `noindex`). Verrouillé par `tests/sitemap.test.js`.
...
Grille 3 colonnes décalée de la bande photo, repliée en colonne unique sous 760px. **Cadre 4/5 sur toutes les tailles depuis le 2026-08-25** : le mobile passait auparavant en 3/2, si bien que le desktop rognait les côtés pendant que le mobile rognait le haut et le bas — aucun cadrage source ne pouvait convenir aux deux. Les photos étant des clichés smartphone en portrait 3/4, le cadre 4/5 ne leur retire que 6 % de hauteur (contre 20 à 40 % de largeur auparavant).
...
## src/index.njk
Accueil : **hero 3D** (`composants/hero-3d.njk`, maquette WebGL + titre en 3 lignes révélées, CTA devis + appel ; photo de fond `chantier-en-travaux.jpg`), puis **bande photos « preuve »** (3 vraies photos de pièces livrées, front matter `photosPreuve`), **séquence de chantier en 3 temps** (`composants/sequence-chantier.njk`, front matter `etapesChantier`), **bloc cas chiffrés**, **encart climatisation** (`composants/encart-climatisation.njk`), frise process, aperçu réalisations, pour qui (colonne investisseurs pointe vers `/retour-client/`, "Voir des cas chiffrés"), garanties (sceau motif), CTA final ("Discutons de votre projet", boutons appel + devis via `.boutons-groupe`).

**Passe photos du 2026-08-25** — la page présentait cinq défauts corrigés d'un bloc, tous verrouillés par `tests/photos-accueil.test.js` :
- La bande « Chantiers réels » montrait 3 rendus IA sous un chapeau promettant « pas de rendu d'agence », dont **une ruine non rénovée** légendée « avant intervention » sous un titre annonçant des pièces livrées. Remplacés par 3 photos réelles de `photos_farid/` ; chapeau réécrit.
- Le comparateur avant/après opposait un **sol carrelé en cours de pose** (675×1200) à un **meuble vasque** (900×1200) : ni la même pièce, ni le même angle, ni le même format. Remplacé par la séquence en 3 temps.
- `cuisine-renovee.jpg` servait à la fois de fond de hero et de 1ʳᵉ photo de la bande, et `mehun-sur-yevre-apres.jpg` apparaissait dans le comparateur **et** dans une carte. Plus aucun doublon.
- La publicité climatisation était **une page entière aplatie en PNG**, servie à 420 px pour une source de 1080 px : texte à ~7 px, invisible pour Google et les lecteurs d'écran. Refaite en HTML/CSS.
- La variable de front matter s'appelle `etapesChantier` et non `sequenceChantier` : ce dernier nom entrerait en collision avec la macro importée du même nom.
...
Macro `sliderAvantApres(avant, apres, titre, prioritaire, variante)`. Le `clip-path` initial est écrit en dur dans le style inline pour que le curseur soit au milieu **avant même que le JS tourne**.

**À n'utiliser que sur un couple réellement cadré à l'identique.** Un wipe entre deux prises de vue de rapports différents rogne les deux calques de façon inégale : plus rien ne se superpose et le curseur ne compare plus rien. C'est le défaut constaté le 2026-08-25 sur l'accueil **et** sur la fiche Saint-Florent. `tests/photos-accueil.test.js` refuse désormais tout couple dont les rapports s'écartent de plus de 5 %. Pour des photos prises à des moments et sous des angles différents, utiliser `composants/sequence-chantier.njk`. La variante `slider-avant-apres--portrait` a été retirée avec le comparateur de l'accueil.
...
Fiche projet individuelle, générée par pagination 11ty sur `_data/realisations.json`. **Quatre** états, du plus riche au plus pauvre : séquence de chantier si `projet.sequence` existe (cas `maison-mitoyenne-mehun-sur-yevre`) ; sinon slider avant/après si les deux photos existent **et sont cadrées à l'identique** ; sinon photo "après" seule dans `.projet__photo-seule` (cas `local-commercial-transforme-saint-florent`) ; sinon fallback placeholder-photo. Curseur du slider avec `role="slider"`/`tabindex="0"`/`aria-value*` (piloté au clavier par `slider-avant-apres.js`). Le `<title>` de page utilise une variante de `projet.titre` sans le tiret cadratin interne (`replace(' — ', ', ')`) pour éviter le double tiret avant « — Soly'bat ». Photo "avant" (ou "après" seule) en `fetchpriority="high"` (candidat LCP de la page), les calques du slider en `decoding="async"`.
...
Données des projets (6 exemples) : slug, type, ampleur, photos, corps de métier, et facultativement `sequence`.

Trois modes d'affichage, consommés par `realisations/projet.njk` :
- **`sequence`** (liste de `{ image, alt, moment, legende }`) — `maison-mitoyenne-mehun-sur-yevre`. Passée en séquence le 2026-08-25 : son couple avant/après opposait une **pose de carrelage en cours** (675×1200) à un meuble vasque (900×1200), deux prises de vue sans cadrage commun que le comparateur à curseur ne pouvait pas superposer. Les 3 photos viennent du même chantier de `photos_farid/` (chantier 08).
- **`photos.avant` + `photos.apres`** — les 4 premiers projets, dont les couples sont des rendus IA générés **par paires**, donc réellement cadrés à l'identique : le comparateur y fonctionne. `tests/photos-accueil.test.js` vérifie que leurs rapports ne s'écartent pas de plus de 5 %.
- **`photos.apres` seule** — `local-commercial-transforme-saint-florent`. Son « avant » était un rendu IA de devanture (carré) face à une vraie photo de cuisine (portrait) : retiré le 2026-08-25, archivé dans `photos/_remplacees-par-photos-reelles/`. **Un vrai « avant » de ce chantier reste à collecter.**

Les photos réelles proviennent de `photos_farid/` (lot de chantiers tiers fourni par l'utilisateur, sans lien vérifié avec un chantier Soly'bat identifié — voir `photos_farid/CONTENU.md`) : à remplacer par de vraies photos Soly'bat dès qu'elles seront disponibles. Les projets mehun et saint-florent citent explicitement "maçonnerie" et "plâtrerie (placo)" dans `corpsDeMetier`, absents des 4 premiers, suite au retour client demandant plus d'explicite sur les corps de métier.

## src/images/realisations/
Photos de chantier en **JPEG** (converties depuis PNG le 2026-07-19, qualité 82 : 23,4 Mo → 2,5 Mo, -89% — c'étaient des photos, pas des aplats, le PNG était le mauvais format).

**7 photos réelles importées le 2026-08-25** par `outils/importer-photos-reelles.py` depuis `photos_farid/` : `cuisine-livree.jpg`, `sejour-pierre-apparente-livre.jpg`, `salle-eau-livree.jpg` (bande « preuve » de l'accueil), `mehun-sur-yevre-arrivee.jpg`, `-pendant.jpg`, `-livraison.jpg` (séquence en 3 temps) et `chantier-en-travaux.jpg` (fond du hero de l'accueil, seul paysage du lot). Le script ne réagrandit jamais une source : `cuisine-livree` et `salle-eau-livree` restent en 480×640, ce qui suffit pour les ~300 px d'affichage de la bande. Plus 3 photos réelles antérieures (`mehun-sur-yevre-apres.jpg`, `saint-florent-apres.jpg`).

**4 rendus IA retirés le 2026-08-25** vers `photos/_remplacees-par-photos-reelles/` (voir le `CONTENU.md` du dossier pour le détail de chaque retrait) : `cuisine-renovee.jpg`, `sejour-renove.jpg`, `piece-a-renover.jpg`, `mehun-sur-yevre-avant.jpg`, plus `saint-florent-avant.jpg`. Les rendus IA restants alimentent les heros des autres pages et les couples avant/après des 4 premières réalisations — voir `docs/prompts-photos.md` pour le mapping des 18 emplacements du lot initial. Ils sont passés par `post_traitement.py` (`--intensite moyenne`) pour un rendu plus crédible "photo de chantier au smartphone". Versions antérieures : `photos/_avant-correction-ia/`, `photos/_avant-post-traitement/`, `photos/_avant-compression-jpeg/`.

`src/images/signature-sceau.png` (180×180, hors de ce dossier réalisations) : sceau détouré en transparent (recadré depuis `logo-solybat.png` en évitant l'artefact de type filigrane connu près du bord droit), utilisé par la signature email dans `identite-visuelle/`.
...
## src/css/styles.css
Point d'entrée CSS, `@import` de tous les fichiers de composants/pages (inclut désormais `placeholder-photo.css`, `hero-photos.css`, `bandeau-nouveaute.css`, `photos-vrac.css`, `retour-client.css`, `barre-appel.css`, `sequence-chantier.css` et `encart-climatisation.css` — ce dernier remplace `publicite-climatisation.css`, supprimé).

## src/_includes/composants/encart-climatisation.njk
Encart climatisation de l'accueil, **en HTML depuis le 2026-08-25**. Il remplace `src/images/publicite-climatisation.png`, une page entière aplatie en image : servie à 420 px pour une source de 1080 px, son texte tombait à ~7 px et le prix « 2 500 € HT » — l'argument commercial le plus fort de la page — n'était ni indexable, ni sélectionnable, ni lisible par un lecteur d'écran (alt vide). Le PNG portait aussi son propre logo et son propre pied de page, qui redoublaient l'en-tête et le footer du site.
- encartClimatisation(lienDevis, telephone, telephoneLien)

Le visuel reste disponible pour les réseaux sociaux et l'impression dans `identite-visuelle/publicite-climatisation.png` (+ sa source `.html`) ; seule la copie de `src/images/` a été supprimée, pour ne plus l'envoyer sur le serveur à chaque mise en ligne.

## src/css/encart-climatisation.css
Deux colonnes (argumentaire / aplat terracotta portant le prix et le CTA), repliées sous 860px. Deux réglages sont là pour le contraste et **ne doivent pas être « simplifiés » en revenant à des blancs translucides** : le label de l'offre est en crème plein (à 85 % d'opacité le rapport tombait à 4,07:1) et la pastille TVA est posée sur un voile **noir** à 18 % (un voile clair éclaircissait le terracotta et faisait tomber le texte crème à 3,78:1). Vérifiés par `outils/verifier-contrastes.py`.

## src/_includes/composants/sequence-chantier.njk / src/css/sequence-chantier.css
Séquence d'un même chantier en plusieurs temps (arrivée, travaux, livraison), en remplacement du comparateur à curseur là où les prises de vue ne partagent pas le même cadrage. Cadre 4/5 comme la bande photos, trait de liaison terracotta entre les étapes (masqué dès l'empilement mobile), surcharges `.section--sombre` prévues. **Pas de numérotation 01→03** : `BRAND.md` la réserve à la frise du process.
- sequenceChantier(etapes, prioritaire) — etapes = [{ image, alt, moment, legende }]
...
## outils/importer-photos-reelles.py
Import des photos réelles de `photos_farid/` vers `src/images/realisations/` : renommage parlant, conversion en `.jpg`, respect de la rotation EXIF (sans `exif_transpose`, une photo portrait ressort couchée au réencodage), côté long plafonné (1200 px, 1600 px pour le fond de hero) et **aucun agrandissement** — une source 480 px reste à 480 px. Idempotent. Modifier la table `PHOTOS` puis relancer.

## outils/faire-archive.py
Fabrique `solybat-a-televerser.zip`, l'archive a decompresser a la racine du dossier web de l'hebergeur (`public_html` / `www`) : le contenu de `_site/` est place a la **racine** de l'archive, sans dossier intermediaire. **Le script efface `_site/` avant de rebatir** — c'est tout son interet : 11ty ne nettoie pas son dossier de sortie, donc un fichier retire des sources y survit indefiniment et repartirait en ligne. Le 2026-08-25, cinq photos retirees et un PNG de publicite auraient ete televerses de cette facon. Verifie ensuite la presence des fichiers recopies par le passthrough (`.htaccess`, `styles.css`, les deux `.php`, `robots.txt`, `sitemap.xml`), qui sont precisement ceux qui disparaissent en silence quand la config d'11ty est mal chargee (voir le piege toolchain de `CLAUDE.md`). L'archive est dans `.gitignore`.

## outils/verifier-contrastes.py
Vérification WCAG des combinaisons texte/fond, y compris **à travers les couches semi-transparentes**, où l'erreur est la plus facile à commettre. Chaque audit précédent avait introduit une régression de contraste pour la même raison : une couleur calibrée dans un sens (crème SUR terracotta) réutilisée dans l'autre. Le jeu contient un témoin qui **doit** échouer (terracotta standard sur crème, 4,01:1), pour que le script ne puisse pas passer au vert en cassant son propre calcul. Sortie non nulle s'il y a un échec.

## tests/photos-accueil.test.js
Garde-fous posés le 2026-08-25, sur des défauts qu'aucun build ni test ne signalait : 11ty génère sans broncher un `<img>` vers un fichier absent, affiche deux fois la même photo sans rien dire, et ignore qu'une légende « avant intervention » contredit le titre de sa section. Vérifie donc : toute photo référencée dans `src/**/*.{njk,json}` existe sur le disque (indexée par nom de fichier, les photos étant réparties entre `realisations/` et `photos-vrac/`) ; aucun doublon sur l'accueil, fond de hero compris ; aucune légende d'état « avant » dans la bande des pièces livrées ; les 3 moments de la séquence dans l'ordre ; et **aucun couple avant/après dont les rapports s'écartent de plus de 5 %** — c'est ce dernier test qui a révélé que la fiche Saint-Florent souffrait du même défaut que l'accueil.
