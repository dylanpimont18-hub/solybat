...
Données des projets (6 exemples) : slug, type, ampleur, photos, corps de métier. Les 4 premiers ont des photos avant/après démo (JPEG) ; les 2 derniers (`maison-mitoyenne-mehun-sur-yevre`, `local-commercial-transforme-saint-florent`, ajoutés suite au retour client "manque des réalisations") ont `photos: {}` et retombent sur le placeholder — à remplacer par de vraies photos comme les autres dès qu'elles sont collectées. Ces 2 nouveaux projets citent explicitement "maçonnerie" et "plâtrerie (placo)" dans `corpsDeMetier`, absents des 4 premiers, suite au retour client demandant plus d'explicite sur les corps de métier.
