...
## src/_data/site.json
Données globales : `url` (placeholder `https://www.solybat.fr`, utilisé pour canonical/OG/sitemap.xml/robots.txt, toujours à remplacer), `telephone`/`telephoneLien` (toujours fictifs, `02 48 00 00 00`), `email` (réel depuis 2026-07-20 : `solybat@gmail.com`, communiqué par le client) — utilisées dans le footer, les mentions légales et le JSON-LD.
...
Données des projets (6 exemples) : slug, type, ampleur, photos, corps de métier. Les 4 premiers ont des photos avant/après démo générées par IA (JPEG). `maison-mitoyenne-mehun-sur-yevre` a désormais un vrai couple avant/après (`mehun-sur-yevre-avant.jpg`/`-apres.jpg`, salle de bain) ; `local-commercial-transforme-saint-florent` n'a qu'un "après" (`saint-florent-apres.jpg`, cuisine — pas de vrai "avant" disponible pour ce chantier), géré par le fallback photo-seule de `projet.njk`. Ces 2 photos viennent de `photos_farid/` (lot de photos réelles de chantiers tiers, fournies par l'utilisateur, sans lien vérifié avec un chantier Soly'bat identifié — voir `photos_farid/CONTENU.md` pour l'inventaire complet et les réserves de provenance) : à remplacer par de vraies photos Soly'bat dès qu'elles seront disponibles. Ces 2 nouveaux projets citent explicitement "maçonnerie" et "plâtrerie (placo)" dans `corpsDeMetier`, absents des 4 premiers, suite au retour client demandant plus d'explicite sur les corps de métier.

## src/images/realisations/
18 photos réelles/générées de chantier (voir historique ci-dessous), désormais en **JPEG** (converties depuis PNG, qualité 82 : 23,4 Mo → 2,5 Mo au total, -89%, aucune perte visible à l'écran — c'étaient des photos, pas des aplats, le PNG était le mauvais format), + 3 nouvelles photos réelles (`mehun-sur-yevre-avant.jpg`, `mehun-sur-yevre-apres.jpg`, `saint-florent-apres.jpg`, redimensionnées à 1200px de long côté et recompressées qualité 82 depuis `photos_farid/`, cf. ci-dessus). Référencées par `image:`/`photos.avant`/`photos.apres` dans les front-matters et `realisations.json` — voir `docs/prompts-photos.md` pour le mapping (18 emplacements générés, tous pourvus : hero accueil ×3, hero renovation-location ×3, hero réalisations ×1, hero espace-pro ×3, + avant/après des 4 premières réalisations). `cuisine-renovee.jpg` (accueil, régénérée) est volontairement laissée nette/léchée pour suggérer une photo prise par un professionnel sur la page d'accueil ; `sejour-renove.jpg`/`piece-a-renover.jpg` (autres photos hero de l'accueil) suivent la même logique. Les 15 autres fichiers générés sont passés par `post_traitement.py` (`--intensite moyenne`) pour un rendu plus crédible "photo de chantier au smartphone" (grain, léger flou, inclinaison + recadrage asymétrique, exposition irrégulière, vignettage, artefacts JPEG). Versions antérieures : `photos/_avant-correction-ia/` (artefact IA retiré), `photos/_avant-post-traitement/` (avant le passage smartphone), `photos/_avant-compression-jpeg/` (PNG originaux avant conversion JPEG).
...
## traiter-devis.php
Handler POST du formulaire de devis, copié tel quel vers `_site/`. Rejette le honeypot silencieusement (redirection succès sans envoi), redirige vers `/devis/?erreur=1&...` avec les valeurs saisies repassées en query string si la validation serveur échoue (page `/devis/` statique, relu côté client par `form-devis.js`), sinon envoie l'email via `mail()` et redirige vers `/devis-merci/`. `$adresseContact` est désormais la vraie adresse (`solybat@gmail.com`, cohérente avec `src/_data/site.json > email` — PHP n'a pas accès à `site.json` en production, seul `_site/` est déployé, d'où la duplication). L'en-tête `From:` reste en revanche `no-reply@solybat.fr` (domaine placeholder) : ne pas le remplacer par `solybat@gmail.com` en "From" — un hébergement mutualisé (OVH) qui envoie un mail prétendant venir de gmail.com sans passer par les serveurs Google se fait généralement bloquer/spammer par Gmail (SPF/DKIM). À aligner sur le vrai nom de domaine une fois connu. **Non vérifié avec `php -l`/exécution réelle : PHP n'est pas installé sur la machine de dev** — code recopié du plan déjà réalisé (`docs/superpowers/plans/2026-07-17-site-solybat.md` Task 18), à valider une fois PHP disponible (local ou sur l'hébergement).
